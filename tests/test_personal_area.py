...
@pytest.mark.order(7)
def test_enter_to_personal_area(site, wait, user_credentials):
    LOGIN(site, wait, user_credentials)  # Do login
    ...
    wait.until(EC.element_to_be_clickable((By.XPATH, locators.profile_save_btn)))
    assert site.find_element(By.XPATH, locators.profile_name).get_attribute('value') == 'VIP client'


@pytest.mark.order(8)
def test_pass_to_constructor_from_personal_area(site, wait, user_credentials):
    LOGIN(site, wait, user_credentials)  # Do login
    # Enter to personal area
    wait.until(EC.element_to_be_clickable((By.XPATH, locators.header_personal_area_btn)))
    site.find_element(By.XPATH, locators.header_personal_area_btn).click()
...
@pytest.mark.order(9)
def test_pass_to_constructor_by_logo(site, wait, user_credentials):
    LOGIN(site, wait, user_credentials)  # Do login
    # Enter to personal area
    wait.until(EC.element_to_be_clickable((By.XPATH, locators.header_personal_area_btn)))
    site.find_element(By.XPATH, locators.header_personal_area_btn).click()
...
def test_logout_from_personal_area(site, wait, user_credentials):
    LOGIN(site, wait, user_credentials)  # Do login
    # Enter to personal area
    wait.until(EC.element_to_be_clickable((By.XPATH, locators.header_personal_area_btn)))
    site.find_element(By.XPATH, locators.header_personal_area_btn).click()
