...
def test_constructor_choose_bread_category(site, wait):
    # Wait until the category buttons is clickable and then click on sauce (for scroll), click on bread for return
    wait.until(EC.element_to_be_clickable((By.XPATH, locators.constructor_bread_btn)))
...
def test_constructor_choose_sauce_category(site, wait):
    # Wait until the category buttons is clickable and then click on sauce
    wait.until(EC.element_to_be_clickable((By.XPATH, locators.constructor_sauce_btn)))
    site.find_element(By.XPATH, locators.constructor_sauce_btn).click()
    time.sleep(2)  # Wait for scrolling finish
...
def test_constructor_choose_filling_category(site, wait):
    # Wait until the category buttons is clickable and then click on filling
    wait.until(EC.element_to_be_clickable((By.XPATH, locators.constructor_filling_btn)))
